...
**File:** `03_WORKFLOW/NH_MASTER-21_TO_FINAL_SYSTEM_DESCRIPTION_ROUTE_v0_2_CANDIDATE.md`
**Status:** CANDIDATE — not accepted, not adopted. Pending independent audit and Ness's acceptance.
**Revision note (v0_2):** v0_1 (SHA-256 `7c80a325adf532868396acebf95296eccc4ac938412659de0aa668442c245897`) is preserved unchanged. This version makes one bounded correction, to §5, replacing wording that described the sequencing difference with the older workflow as unresolved and assigned its reconciliation to a separate later Ness decision. Ness has already selected the route recorded here. The self-audit wording in §7 is adjusted correspondingly. Nothing else is changed.
...
That file (2026-06-30) remains preserved and unedited by this one, and is not superseded by it.

**Ness's current instruction selects the sequence recorded here for this work:** Master-21 adoption → gap decisions → finished description.

One sequencing difference from the older workflow is recorded, for traceability:
...
Recording that difference does not make it an open question. Phase 18's sequence does not override the route selected above and does not introduce an additional prerequisite to it. The older workflow file stays in place, unedited, and keeps its own scope; no reconciliation between the two sequences is outstanding.

This section records a workflow choice Ness has already made. It does not perform Master-21 adoption, does not accept this candidate, and does not authorize building. Actual Master-21 adoption and permission to build remain separate explicit Ness actions, as §1.3 and §4 state.
...
- §5 records the sequencing difference from workflow v1.0 for traceability only; the route is stated as a choice Ness has already made, not as an open question, and no reconciliation is left outstanding or assigned to a later decision.
- No planned event is reported as completed; Stage 1 adoption is marked planned in §1.3, and §0 states that none of the described events has occurred. Recording an already-made workflow choice is distinguished throughout from performing adoption or authorizing building, both of which remain separate explicit Ness actions.
...
- v0_1 is preserved unchanged at SHA-256 `7c80a325adf532868396acebf95296eccc4ac938412659de0aa668442c245897`; this version was produced as a new file and v0_1 was not edited in place.
- Correction scope held to §5, the file/version identification, the revision note, and this self-audit. No other text differs from v0_1.
