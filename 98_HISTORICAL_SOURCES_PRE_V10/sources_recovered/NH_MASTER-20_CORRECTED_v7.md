...
`tsc_retained_archive_created` written; session → `"archived"`.
The session does not remain in `"promoted"` after a retained archive is
successfully created. Blocked items remain in `promotion_state` as pending,
resuming automatically when their blockers clear through normal §7E rules —
