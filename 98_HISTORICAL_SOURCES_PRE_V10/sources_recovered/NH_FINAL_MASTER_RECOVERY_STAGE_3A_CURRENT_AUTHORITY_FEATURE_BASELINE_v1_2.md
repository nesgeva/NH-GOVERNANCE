# N.H — FINAL-MASTER RECOVERY — STAGE 3A: CURRENT-AUTHORITY FEATURE BASELINE v1.2

**Narrowly corrected successor to Stage 3A v1.1.** v1 and v1.1 are both preserved unchanged at
`…_v1.md` and `…_v1_1.md`. v1.2 is a documentation-and-packaging copy of v1.1: it applies only label
normalization, a truthful restatement of what was read, and the two-file-baseline framing below. It does
**not** reread sources, regenerate feature recovery, change any recovered feature/behavior/status/
integration judgment/citation range/architecture decision/unresolved question, or begin Stage 3B.

**Stage 3A is a TWO-FILE baseline — read v1 and v1.2 together.**
- **Stage 3A v1** contains the complete Master-side ten-field feature inventory (subsystems A–AB) and the
  plain-language behavior map.
- **Stage 3A v1.2** contains the corrected statuses (§1) plus the complete security (§3A), TSC (§3B),
  governance (§4), and Cursor Rules (§5) recovery.
- **The authoritative Stage 3A audit result is therefore `v1 + v1.2 read together`.** v1.2 is **not** a
  self-contained replacement: it does not restate the v1 Master-side ten-field entries or the v1 behavior
  map — those live only in v1 and are corrected here by status, not reproduced.

**Why v1 was provisional (recap).** v1's opening claimed the complete bodies of all six sources were read,
but v1's own §6 showed several were read only in part. The v1.1 turn corrected this by reading the
companion bodies, Cursor Rules, and the governance Part I workflows directly; v1.2 inherits that recovery
unchanged and only states the reading truthfully (see §8).

**What this is NOT.** No source reread; no feature-recovery regeneration; no historical-Master comparison;
no conflict resolution; no choice made for Ness; no authority-file patch; no final-Master organization; no
canonical drafting; Stage 3B not begun.
...
| Cursor Rules governance (E1–E3) | SRC-076 (whole file) | BUILT, BUT NOT FRESHLY DISK-VERIFIED IN THIS AUDIT | (superseded by §3A direct recovery) | re-derived directly |
| Legacy REALITY/SIMULATION gate (F1) | Master §5/§6A; SRC-076 §2A | BUILT, BUT NOT FRESHLY DISK-VERIFIED IN THIS AUDIT | BUILT, BUT NOT FRESHLY DISK-VERIFIED IN THIS AUDIT | unchanged |
...
## 8. SOURCE-RANGE VERIFICATION REPORT — REWRITTEN, TRUTHFUL (corrects v1 §6; updated in v1.2)

This audit is document-based and performed **no fresh disk inspection**. The reads below were performed
during the v1.1 turn; v1.2 does not reread. The table states exactly what was read — including, for the
governance companion, what was **not** read in full.

| Source | Path read | Body size | What was actually read in Stage 3A |
|---|---|---|---|
| SRC-044 Master v7_1 | `/mnt/project/NH_MASTER-19_CORRECTED_v7_1.md` | 1937 L | **Ranges read directly: L174–1740 and L1759–1937** (premise→§7R, §8–§11 incl. mid-items and §11-SETTLED, §12–§16 model layer, §17–§18 logs, §19A–E, §22–§25, closing principles, truest sentence). **Not read line-by-line:** the front matter **L1–173** (title, HOW-TO-READ block, status table, §2 how-to-work, §3 evolution) and the short span **L1741–1758** — both covered indirectly via the §20/§21/§25 change-logs and the status-table content rather than read line-by-line. The listed ranges do **not** cover all 1,937 lines; this is **not** a claim that every section was read line-by-line. |
| SRC-015 DD-S19_v2_2 | `/mnt/project/NH_DECISION_DEFAULTS-S19_v2_2.md` | 314 L (manifest) | **Complete body read.** The governance manifest records 314 lines; the viewer reported "315 lines total" because the file ends with a trailing newline, so the displayed range was L1–315 over a 314-line body. Content read: §0–§6, including §3A–§3N, §4 build-state, §5 open forks, §6 first-class principles. |
| SRC-076 Cursor Rules v3.2 | `/mnt/project/cursorrules__1_` | 717 L | **Complete body read** (L51–717 content + L1–50 preamble): IDENTITY, §§1A/1B/1C, §2A, §2B, §3A/3B, §4A/4B, §5, §6, §7, §8, §9, §10, §11, §12A–D, REMINDER. Recovered directly from the file, not via the Master §6A summary. |
| SRC-087 governance companion | `/mnt/project/NH_PROJECT_COMPANION_GOVERNANCE_AND_ARCHIVE_v1.md` | 5580 L | **Partial — the 5,580-line body was NOT read in full.** Read directly **in full:** the header, authority order, files-separate list, embedded-source preservation method, and source manifest (L1–45), and **Part I workflows in full** (L47–303: shared handoff + working roles). **SRC-001 and SRC-002, embedded in Parts II and III, were read completely** — but through their **standalone byte-identical bodies** (the next two rows), not by reading Parts II/III in place; the Part II/III openers and overall structure were mapped. **Parts IV (wellbeing design-source snapshot) and V (AFTER_BGMM handoff + a superseded MASTER-19_FULL) were identified and classified as historical/archive material but were NOT read completely** during Stage 3A. |
...
**Honest scope summary.** Five of the six sources were read completely (Master ranges as listed above —
near-complete but not every line; DD complete; Cursor Rules complete; SRC-001 complete; SRC-002 complete).
The **governance companion (SRC-087) was read only in part**: its current-governance front matter and Part
I in full, its embedded accepted designs in full via the standalone companion bodies, and its Parts IV–V
left as classified-but-unread historical archive. Do **not** read this report as a claim that the full
5,580-line SRC-087 body or every line of the Master was read.

**Correction of v1's §6 defects (retained).** v1's report cited governance-embedding ranges (`gov L…`) as
if they were the companions' direct ranges and claimed full reads that were partial. The recovery cites
source-body-relative ranges throughout and uses `gov L…` only as a secondary cross-reference (and, in §4,
as the direct source for the governance companion's own Part I).

---

## 9. v1 → v1.1 → v1.2 CORRECTION LEDGER (correction #8)
...
§6 (now §8) rewritten so the reading report is truthful — five sources read completely (Master by the
listed ranges, near-complete but not every line; DD, Cursor Rules, SRC-001, SRC-002 complete) and the
governance companion read only in part (front matter + Part I in full; embedded SRC-001/SRC-002 in full
via their standalone bodies; Parts IV–V classified but unread); impossible/mismatched ranges removed.
...
*The four blocks above (preserved / newly recovered / status corrections / citation corrections / scope
corrections) record the v1 → v1.1 substantive work, carried into v1.2 unchanged.*

**v1.1 → v1.2 documentation and packaging corrections (this turn; no source reread, no recovery
regenerated):**
- **Two-file baseline framing made explicit:** Stage 3A's authoritative result is `v1 + v1.2 read
  together` — v1 holds the Master-side ten-field inventory and behavior map; v1.2 holds the corrected
  statuses plus the security/TSC/governance/Cursor recovery. v1.2 is stated to be **not** a self-contained
  replacement (it does not restate the v1 Master-side entries).
- **Full-body-reading claims corrected to match §8 truthfully:** the intro and §8 no longer say "all six
  complete bodies were read." SRC-087 is stated as read only in part (front matter + Part I in full;
  embedded SRC-001/SRC-002 in full via standalone bodies; Parts IV–V classified but unread) — the
  5,580-line body was **not** read in full. The Master row now lists only the ranges actually read
  (L174–1740, L1759–1937) and explicitly disclaims "every line / every section line-by-line." The DD
  line-count inconsistency is explained (314-line body; viewer showed L1–315 due to a trailing newline).
- **Status-label normalization:** the two status-table cells reading `BUILT, BUT NOT FRESHLY DISK-VERIFIED`
  now read the exact allowed label `BUILT, BUT NOT FRESHLY DISK-VERIFIED IN THIS AUDIT`; every field-9
  value begins with exactly one of the eleven allowed labels, with explanations kept outside the label.
- **No substantive change:** no recovered feature, design behavior, accepted status, integration judgment,
  citation range, architecture decision, or unresolved question was altered.
...
## STATE AFTER STAGE 3A v1.2 (nothing decided, nothing changed)
- **v1 and v1.1 both preserved unchanged;** authority unchanged (v7_1 / DD-v2_2 / Cursor Rules v3.2 /
  governance companion v1).
- **Stage 3A is a two-file baseline: read `v1 + v1.2` together.** v1 = Master-side ten-field inventory +
  behavior map; v1.2 = corrected statuses + the security/TSC/governance/Cursor recovery. v1.2 is not a
  self-contained replacement.
- Five sources were read completely (Master by the listed ranges — near-complete, not every line; DD;
  Cursor Rules; SRC-001; SRC-002); the governance companion (SRC-087) was read only in part (front matter
  + Part I in full; embedded SRC-001/SRC-002 in full via their standalone bodies; Parts IV–V classified
  but unread). The two accepted companions and the governance/Cursor workflows are fully recovered in
  ten-field form; statuses and citations corrected; labels normalized.
- No source reread; no feature-recovery regenerated; no historical Master compared; no conflict resolved;
  no design judged or changed; nothing integrated, patched, or decided; no organization chosen; no
  canonical Master drafted; Stage 3B not begun.

**STAGE 3A v1.2 COMPLETE. AWAITING NESS.**
