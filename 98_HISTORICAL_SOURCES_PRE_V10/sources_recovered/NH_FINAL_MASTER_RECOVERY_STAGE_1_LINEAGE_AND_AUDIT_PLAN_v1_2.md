# N.H — FINAL-MASTER RECOVERY — STAGE 1: LINEAGE AND AUDIT PLAN v1.2

**This is the corrected successor to v1.1.** It applies only planning-level corrections (see the
Correction Ledger at the end). v1.1 is preserved unchanged. No architecture, source content,
comparison evidence, hash, byte count, or settled N.H decision was altered.

**Scope of this file.** This is a *planning and lineage* document only. It establishes what exists, a
*provisional* hypothesis of how the versions may descend from one another, how the files separate by
role, a *provisional comparison queue (pending chronology verification)*, and a *provisional Stage-1*
feature-category map of the N.H architecture. **The lineage and queue below are an evidence-ordering
hypothesis, not a proven chronology** (see §1A).
...
## 0A. CHRONOLOGY IS A HYPOTHESIS UNTIL VERIFIED (governs all later stages)

**The lineage in §2–§3 and the comparison queue in §5 are a provisional comparison *hypothesis*, not a
proven chronological sequence.** They were built from version tokens, filenames, copy suffixes, file
sizes, and upload provenance — none of which can *prove* order on their own.

**Before the Stage 2 feature comparisons begin, a chronology-verification pass must establish order
from internal evidence:**

- internal creation and update dates;
- session numbers and session declarations;
- explicit predecessor/successor statements;
- embedded change logs;
- statements describing what was added to, or regenerated from, another file;
- content relationships;
- provenance records.

Filename numbering, copy suffixes, file size, upload order, and labels such as `FINAL`, `FULL`, or
`CORRECTED` **may support an inference but may never prove chronology by themselves.** Any relationship
that cannot be established from the evidence above **must remain `UNCLEAR`.**

This pass only fixes *when designs appeared, changed, disappeared, or returned*. It does **not** decide
the eventual canonical Master's reading structure (see §5A) and does not begin in this file.
...
- `NH_PROJECT_COMPANION_GOVERNANCE_AND_ARCHIVE_v1.md` — **governance, accepted-design, and provenance
  authority within its declared scope; explicitly NOT an independent architectural authority** and not
  a replacement for the architectural Master. **A design explicitly accepted by Ness remains accepted
  and governing within its declared scope even while it is stored here and not yet integrated into the
  Master.** Patching does not create the acceptance; patching integrates an already-accepted design
  into the self-contained architectural Master (see §4.5 for the full rule).
...
## 2. MASTER LINEAGE (PROVISIONAL HYPOTHESIS — reconstructed from filenames, sizes, and declared content)

**Reconstruction basis.** This lineage is built from filename version tokens, byte/line sizes
(evidence, not verdict), and the preservation provenance. **It is a provisional ordering hypothesis for
the comparison plan, not a proven chronology and not a decision about which body wins.** Per §0A, the
order shown here is **not yet verified**; it must be confirmed against internal dates, session
declarations, explicit predecessor/successor statements, embedded change logs, content relationships,
and provenance before any Stage 2 comparison relies on it. Where ordering between near-siblings cannot
be settled from internal evidence, it is marked **UNCLEAR — confirm by content in audit.**
...
## 3. DECISION-DEFAULTS LINEAGE (PROVISIONAL HYPOTHESIS)

Same basis and same caution as §2 and §0A. Provisional ordering hypothesis for the comparison plan
only — not a proven chronology until verified from internal evidence.
...
### 4.5 Accepted companions (ACCEPTED AND GOVERNING WITHIN SCOPE — not yet integrated into the Master)
- `NH_ACCEPTED_SECURITY_IDENTITY_DESIGNS_AFTER_BGMM__1_.md` (SRC-001) — the file's own labels read
  "ACCEPTED DESIGN — NOT YET BUILT" and "must be formally patched into the Master" (its wording,
  preserved as provenance). Contains BOP, Other-Speaker/Guest architecture, SIA, SACL,
  Wellbeing/Identity/Security separation, BAI, Owner-Phone Pairing, Recovery-Code Lifecycle,
  Future-Phone Replacement, Atomic Emergency Recovery, Ness Voice-Profile Enrollment Bootstrap,
  Vocabulary Additions, BGMM.
...
**Acceptance rule (corrected — applies to all accepted companion designs):**
- The governance companion is **not** an independent replacement for the architectural Master.
- A design **explicitly accepted by Ness remains accepted and governing within its declared scope**
  even while it is stored in an accepted-design companion and has not yet been integrated into the
  Master. "NOT YET BUILT" / "not yet patched" describe build/integration state, **not** acceptance.
- **Patching does not create acceptance.** Patching integrates an already-accepted design into the
  self-contained architectural Master.
- Accepted-but-not-yet-integrated designs **must not be downgraded** to candidates, optional material,
  or merely historical evidence.
- Any conflict between an accepted companion design and existing Master wording **must be surfaced to
  Ness**, never silently resolved or discarded.
...
## 5. PROVISIONAL COMPARISON QUEUE — PENDING CHRONOLOGY VERIFICATION

This is the *provisional route* the later audit will walk, drawn from the unverified lineage hypothesis
(§0A, §2–§3). It is presented oldest → newest along each spine, with variant-clusters resolved
internally **before** the cluster is compared to the next spine node. Every step traces
add / weaken / drop / change — never "newer wins" (Rules 1–3). **EXACT DUPLICATE pairs are skipped**
(no content delta possible).

**This queue is pending chronology verification.** It **may be reordered only when internal evidence
(§0A) establishes a more accurate relationship** — not on the basis of filenames, suffixes, sizes,
upload order, or `FINAL`/`FULL`/`CORRECTED` labels. **Reordering must never cause any source to be
skipped:** every source listed here remains in the queue regardless of how chronology resolves; a
chronology correction changes only *position*, never *inclusion*.
...
### 5A. EVIDENCE CHRONOLOGY IS NOT THE FINAL MASTER'S ORGANIZATION

Chronology is required for **tracing when designs appeared, changed, disappeared, or returned**. It
does **not** automatically determine the eventual canonical Master's reading structure.

- The final Master's organization — **architecture-based, chronological, or another structure — has not
  yet been selected by Ness.** No organizational choice may be made on his behalf.
- Whatever main structure Ness later chooses, the final Master must still contain an **accurate
  chronological provenance / change-history section** so the design's history is recoverable regardless
  of how the body is arranged.
- Producing a verified chronology in Stage 2 therefore does **not** imply or pre-commit a
  chronologically-organized Master.
...
- The lineages, role-separation, provisional comparison queue, and provisional feature-category map
  above are the **plan** for the later work; the lineage/queue are an **unverified chronology
  hypothesis** (§0A).

### 9.1 Stage order (corrected)
The sequence from here is fixed as:

1. **Stage 1.2 planning correction** (this file).
2. **Chronology-verification pass** — prove order from internal evidence (§0A); leave unprovable
   relationships `UNCLEAR`.
3. **Full feature-recovery audit** — walk the (possibly reordered) queue in §5; trace
   add/weaken/drop/change; surface conflicts.
4. **Ness's unresolved decisions** — the genuine concept/architecture questions the audit raises.
5. **Ness's decision about final-Master organization** — architecture-based, chronological, or
   another structure (§5A); chosen by Ness, not on his behalf.
6. **Only then, drafting the canonical Master.**

**No canonical Master drafting may begin until the chronology audit, the feature audit, and Ness's
organizational decision are all complete.** Each step waits for Ness's instruction.
...
## 11. CORRECTION LEDGER (v1.1 → v1.2)

v1.1 is preserved unchanged. Each row below is a planning-level correction only. No architecture,
source content, comparison evidence, hash, byte count, line count, or settled N.H decision was changed.

| # | Where | v1.1 said (old) | v1.2 says (replacement) |
|---|---|---|---|
| 1 | new §0A; §2, §3 intros; title/scope | Lineage presented as "reconstructed from filenames, sizes, declared content," implicitly an order to walk | Added §0A: the lineage/queue are a **provisional comparison hypothesis, not a proven chronology**. Before Stage 2 comparison, chronology must be verified from internal dates, session numbers/declarations, explicit predecessor/successor statements, embedded change logs, regeneration/addition statements, content relationships, and provenance. Filenames, suffixes, sizes, upload order, and `FINAL`/`FULL`/`CORRECTED` labels may support but never prove order; unprovable relationships stay `UNCLEAR`. §2/§3 retitled "PROVISIONAL HYPOTHESIS" |
| 2 | §5 heading + intro | "PAIRWISE COMPARISON ORDER FOR THE LATER FEATURE-RECOVERY AUDIT" | Renamed "**PROVISIONAL COMPARISON QUEUE — PENDING CHRONOLOGY VERIFICATION**." Added: the queue may be reordered **only** when internal evidence establishes a more accurate relationship, and **reordering must never cause any source to be skipped** — chronology correction changes position, never inclusion |
| 3 | new §5A | (no separation stated) | Added §5A: evidence chronology traces *when designs appeared/changed/disappeared/returned*; it does **not** determine the final Master's reading structure. The final Master's organization (architecture-based, chronological, or other) is **Ness's later decision, not made on his behalf**; whichever structure is chosen, the final Master must still contain an accurate chronological provenance/change-history section |
| 4 | §1.1 + §4.5 | §1.1: accepted designs "carry Master authority only after being patched" | Corrected acceptance rule: the governance companion is not a replacement for the architectural Master; **a design explicitly accepted by Ness remains accepted and governing within its declared scope even before integration**; **patching integrates an already-accepted design — it does not create acceptance**; accepted-but-not-integrated designs must **not** be downgraded to candidate/optional/merely-historical; any companion-vs-Master conflict must be **surfaced to Ness**, not silently resolved. §4.5 bucket retitled "ACCEPTED AND GOVERNING WITHIN SCOPE — not yet integrated" |
| 5 | §9.1 | "Next stage: begin the feature-recovery audit by walking §5" | Replaced with the fixed stage order: (1) Stage 1.2 planning correction → (2) chronology-verification pass → (3) full feature-recovery audit → (4) Ness's unresolved decisions → (5) Ness's final-Master organization decision → (6) only then canonical drafting. Added the gate: **no canonical drafting until chronology audit, feature audit, and organizational decision are all complete** |

**This file performs none of stages 2–6.** No chronology reconstruction, feature comparison,
integration, conflict resolution, or canonical drafting was done here.

**STAGE 1 v1.2 COMPLETE. AWAITING NESS.**
