...
> | BOP (Behavioral Observation Processing) | **ACCEPTED DESIGN, NOT BUILT** | full event vocabulary; pending `acoustic_condition_notes` amendment NOT accepted — pending separate review; §25 |
...
> | BAI (incl. `authorization_type = "enrollment_declared"`) | **ACCEPTED DESIGN, NOT BUILT** | purpose-bound tokens; adopted vocabulary addition; §25 |
...
> | Corrected TSC specification (no-inspection, no-destruction) | **ACCEPTED WITH LATER CORRECTIONS, NOT BUILT** | inspection prohibited; destructive lifecycle corrected; §7E-TSC |
...
> | Creation-aware Meaning Engine mode | **SETTLED CONCEPT, NOT BUILT** | mode inside §7G; unified Creation Store; §14 |
...
archived          — retained archive only; active TSC structural database sealed
                   and rendered read-only/inaccessible (not deleted)
permanently_sealed — archive permanently sealed (not deleted);
                   history record written documenting the seal authorization
```
...
1. Raw content not held in §7E or TSC at any point. If identified before
   ...
   record retained as a history record.
2. `item_lifecycle_status` → `"excluded"`. `exclusion_metadata` field carries:
   ...
   Excluded raw content moves to separate sealed protected storage under the
   applicable §7Q protection level. The TSC-side record keeps only the
   non-reconstructive metadata, an opaque reference to the protected record,
   and the required handling history.
   stored.
...
[SUPERSEDED — see note below] tsc_archive_deletion_authorized
[SUPERSEDED — see note below] tsc_archive_deleted

**Archive event name corrections (pending review):** The old event names `tsc_archive_deletion_authorized` and `tsc_archive_deleted` are superseded for active use. Their required corrected meanings: authorization to permanently seal the archive (not delete it) and archive permanently sealed with history record written (not deleted with tombstone). Proposed replacement names `tsc_archive_seal_authorized` and `tsc_archive_sealed` are NOT formally adopted vocabulary — they are proposed mechanical replacements pending review during the Master-drafting process. The old names are retained in provenance.
                                 (old semantics: deletion + tombstone — superseded by no-destruction law)
...
**AUTOMATIC PROVISIONAL CREATION RECORDS.** During a live session, creation records are produced automatically as provisional entries. Confirmation occurs through one of: Ness explicitly confirming; Ness clearly adopting or acting on the creation in a later context; a time-based provisional-to-confirmed transition rule (not yet designed).

**STATUS.** SETTLED CONCEPT — NOT BUILT. Exact creation-mode behavior, provisional-to-confirmed rules, and store schema remain undesigned.
...
Within the worker pass sequence, §7F (Context Retrieval) is the first query made through LMAC, not the only possible query. If mid-pass the worker needs current shared state — for example, a current §7Q eligibility confirmation before Computed View processing, or a §7R relevance mode check — it may query LMAC again at that point. LMAC routes the query to the appropriate component and returns the current live state at that moment. LMAC does not replace §7F, §7R, §7Q, or §7P; it routes to them. Privacy and permission rules (§7Q and §7P) apply to every query routed through LMAC, including mid-pass queries.
...
§7Q Layer-1 eligibility applies via LMAC before clash detection operates on the reading. Operation key: `{job_id}::{reading_id}::clash_detection`.
...
Clash detection must complete (checkpoint 7A present) before Computed View snapshot computation begins. §7Q Layer-1 eligibility applies via LMAC before any Computed View operation. The §7R relevance mode appropriate to the view-assembly purpose is applied via LMAC.
...
**Sealed isolation** — material enters N.H but is sealed inside a protected execution boundary (Level 1). It is preserved but never exposed in raw form. Authorized use occurs inside the boundary; only the minimum safe result leaves it. Ordinary records reference the material by opaque identifier only. This is the correct handling for live credentials, secrets, and high-harm material that must be preserved for operational integrity but never displayed, analyzed, or surfaced. Credentials that become invalid (rotated, revoked, expired) are rendered permanently inert — they remain preserved as historical records but can never function again.
...
**Deletion** — non-destructive visibility removal. The root and all material derived from it are removed from ordinary visible output, retrieval, display, analysis, simulation, and use in new derivations. True destruction is prohibited (§0B). Content remains preserved internally, connected, and internally usable unless Ness separately gives an explicit influence-removal instruction. Stopping influence on future outputs is a separate instruction with its own scope and record. A history record documents: what was removed from visible output, when, by what authority, the outcome, and the verification record.
...
DELETION AS NON-DESTRUCTIVE VISIBILITY REMOVAL. True destruction is prohibited (§0B). Deletion removes material from ordinary visible output, retrieval, display, analysis, simulation, and use in new derivations — but content remains preserved internally. N.H may confirm complete visibility removal only when every known output location has been verified clean.
...
Every identified derivative must be removed from visible output, rebuilt without the deleted source's visible contribution, or explicitly marked unresolved if safe removal cannot yet be verified. Multi-root derived objects may be recomputed without the deleted root's visible contribution only if the deleted content is no longer recoverable from the visible output, their meaning is re-evaluated without that evidence, and their provenance records that they were rebuilt. Embeddings and indexes derived from deleted content must be removed from active retrieval and rebuilt where necessary. Backups follow an explicit visibility-removal or retention process.
...
**THE TWO FILTERS (explored S8 — creation filter now settled).** Chat on the right, N.H on the left, two filters in the middle: (1) AI creation filter — catches what is created in the conversation (designs, ideas, decisions); (2) meaning filter — IS the Universal Filter / meaning engine (§7A/§7B). Path: chat → AI creation filter → meaning filter → N.H (memory + log). **SETTLED (June 29 2026):** the creation filter is a creation-aware mode of the one meaning filter (§7G), not a separate mechanism. See §7G CREATION-AWARE MODE.
...
**SETTLED AND REMAINING OPEN (June 29 2026).** Creation filter: SETTLED — a mode of the meaning engine (§7G). Always-capture: SETTLED — every live-chat message is captured automatically; no manual save marker required. Remaining genuinely open: exact Creation Mode implementation details; provisional-to-confirmed transition rules; Creation Store schema and file format.
...
## 27. RECOVERY AND CORRECTION LOG — CURRENT SESSION (June 25 2026)
