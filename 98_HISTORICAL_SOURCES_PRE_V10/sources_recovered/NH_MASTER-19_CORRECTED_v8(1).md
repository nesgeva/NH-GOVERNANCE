...
*Session 18 (June 24 2026) was a focused conceptual-design session for Attention and Relevance Control, designed interactively one decision at a time with Ness and checked by ChatGPT and Ness before adoption. Fourteen decisions were made and checked. No code was written in S18. The component is marked [CORE CONCEPTUALLY DESIGNED, NOT BUILT]. This file (NH_MASTER-19_CORRECTED_v7.md) is the adopted authoritative N.H Master, explicitly adopted by Ness (June 28 2026). It supersedes NH_MASTER-19_CORRECTED_v6.md.*

*★ DESIGN SESSION (June 28 2026): Three foundational laws added. (1) §0A revised: the pure-tape note is replaced with a two-level sensitive-content protection model — Level 1 (live secrets and credentials, sealed execution boundary only, never exposed in raw form) and Level 2 (private personal information, available to Ness after identity verification, securely protected from external access). True destruction prohibited. Safe references and opaque protected-record identifiers introduced. (2) §0B added: the Full-Transparency and Living-Record Law — universal recording of all external information and all internal N.H cognition and operations; every record is active living memory not passive audit; triggered recursive self-examination; use and non-use records mandatory; append-only growth; mandatory connections; relationship to privacy and protected storage detailed. (3) §7Q revised: six operations replacing five (Exclusion, Sealed isolation, Hiding, Restriction, Redaction, Deletion); four sensitivity levels preserved and now mapped to the two protection levels; deletion redefined as non-destructive visibility removal not permanent erasure; earlier no-use-after-deletion proposal explicitly marked REJECTED; influence-removal instruction defined as separate from deletion; five new outcome states replacing five old ones; Layer A rewritten for Level 1 sealed processing boundary. No existing settled design was changed or removed. This file is NH_MASTER-19_CORRECTED_v8.md.*
...
> | Full-transparency and living-record law (§0B) | **DESIGNED (June 28 2026)** | universal recording of all external info and internal N.H operations; active living memory not passive audit; triggered recursive self-examination; two protection levels; deletion = visibility removal not destruction; §0B |
> | Two-level sensitive-content protection (§0A, §0B, §7Q) | **DESIGNED (June 28 2026)** | Level 1 (live secrets, sealed execution boundary, never exposed); Level 2 (private personal info, available after identity verification); true destruction prohibited; opaque protected-record identifiers; §0A |
> | Pure-tape redaction/destruction path | **SUPERSEDED (June 28 2026)** | replaced by two-level protection model and non-destructive deletion in §0A and §7Q; cryptographic erasure and permanent destruction prohibited; see §0A and §7Q |
...
> | Privacy, deletion, and sensitive-data handling | **PARTIALLY CONCEPTUALLY DESIGNED, NOT BUILT** | six operations (exclusion, sealed isolation, hiding, restriction, redaction, deletion-as-visibility-removal); four sensitivity tiers; two protection levels; no-destruction rule; exclusion system; third-party baseline; two-stage access control; private-access presumption designed; detection methods, verification procedures, implementation mechanics undesigned; §7Q |
...
> | Temporary Session Cache (TSC) | **CONCEPTUALLY DESIGNED, NOT BUILT** | transactional extension of §7E pre-ingest holding area; one new blocker type (`"pending_fingerprint_authorization"`); detailed 31-section design accepted; §7E-TSC |
...
> **★ THE PURE TAPE AND SENSITIVE CONTENT — PRESERVATION WITH TWO PROTECTION LEVELS, NOT DESTRUCTION.**
>
> "Append-only" does not mean ordinary visible storage of every secret in every location. It means nothing is erased from N.H's record. Two distinct protection levels make this compatible with the different needs of different types of sensitive material.
>
> **Level 1 — Live secrets and credentials.** Passwords, private cryptographic keys, authentication tokens, recovery codes, one-time security codes, payment-card security codes, and equivalent live access credentials. These must never enter ordinary roots, logs, indexes, embeddings, caches, exports, or model context where ordinary retrieval, chat, interfaces, or ordinary access can reach them. When encountered at a front door, they are intercepted immediately and moved to a sealed protected storage area or protected pre-ingest hold governed by separate authority, encryption, and access rules. Authorized system functions may process them temporarily inside a protected execution boundary when genuinely required. Only the minimum safe result may leave that boundary. The raw content must never be displayed, quoted, exported, summarized, or exposed in raw form under any circumstance, including to Ness. Ordinary records carry only the opaque protected-record identifier and the full handling and use log. The physical location, encryption details, or access path of the sealed area does not appear in ordinary records.
>
> **Level 2 — Private personal information.** Medical records, personal memories, private conversations, personal documents, and similar sensitive life material belonging to Ness. This material is securely stored and protected from unauthorized access, but it is not subject to the strict "never exposed in raw form" rule that governs Level 1. Once N.H successfully verifies that the current user is Ness, his own Level 2 material is available to him by default through normal authorized private interaction — he does not need to give separate permission each time to view, discuss, summarize, or use it. External exposure, exports, third-party access, shared screens, and unauthorized secondary uses remain restricted by default and require explicit authorization. Every access, display, summary, discussion, retrieval, and use is recorded and connected.
>
> **True destruction is prohibited.** No information, record, credential, secret, log, derivative, interpretation, connection, or protected item may ever be permanently erased from all storage locations inside N.H. Cryptographic erasure that destroys the only copy of any content is prohibited. Any system path or user request described as deletion must result only in a non-destructive state change — hiding, restriction, quarantine, sealed isolation, revocation of visible access, or removal from ordinary visible output paths. The original material remains preserved at all times. Hiding or removing something from visible output does not automatically remove its internal usability unless Ness separately instructs that. Every such state change is itself permanently recorded and connected.
>
> **Safe references in ordinary records.** For Level 1 material, and for any Level 2 material specifically placed into sealed isolation, ordinary records outside that protected area must not duplicate or contain the sealed raw content. They contain only an opaque protected-record identifier that references the sealed item without revealing its physical location, encryption details, access path, or storage structure. Ordinary records know that a protected item exists and can reference it by its opaque ID. They do not know or expose where it is stored or how it is reached. The ordinary record must also carry the full handling record required by §0B: that sensitive material was encountered; when and where it entered; which component handled it; why it was classified as protected and at which level; the opaque ID of the sealed record; when an authorized component accessed or used it; why that use was permitted; how it affected the result; and whether access succeeded, failed, or was refused. Level 2 material that has not been placed into sealed isolation may exist in N.H's ordinary protected memory and may appear in authorized visible output to Ness after successful identity verification.
>
> **Authorized internal use inside a protected execution boundary.** An authorized system function may temporarily process the raw Level 1 sealed content inside a protected execution boundary when it genuinely requires it. The raw content does not enter ordinary memory, ordinary logs, ordinary caches, chat, interfaces, exports, unrestricted model context, or any normal visible access path. Processing occurs inside the boundary. Only the minimum safe result that the authorized function needs to produce may leave the boundary. That result must not reconstruct, reveal, or allow inference of the raw sensitive content. Every access — the fact that an authorized function entered the protected boundary, when, why it was authorized, what type of operation was performed, what the minimum result was, and whether access succeeded, failed, or was refused — is logged through the opaque protected-record identifier in the ordinary operational log. The raw content itself does not appear in that log entry.
>
> **Existing security and identity protections are unchanged.** These two protection levels do not weaken or replace identity verification, biometric authorization, SACL, BAI, anti-impersonation, or any other security layer. Those layers govern who may use N.H as Ness. These protection levels govern what sensitive content may be reached during authorized use.
...
## 0B. THE FULL-TRANSPARENCY AND LIVING-RECORD LAW  [DESIGNED — foundational law applying to all components]

**THE LAW IN ONE SENTENCE.** Everything that exists, happens, is decided, is omitted, fails, succeeds, or is used inside N.H must be permanently recorded, connected to everything it relates to, and actively available for N.H's own use — including N.H's use of that record itself.

**SCOPE — NO EXCEPTIONS EXCEPT WHERE §7Q EXPLICITLY GOVERNS.** This law applies universally to:

— every piece of external information entering N.H, in every form and at every stage;
— every piece of N.H's own internal understanding: every reading, interpretation, web-pass result, firmness judgment, theme proposal, clash detection, relevance judgment, retrieval choice, context selection, acceptance, rejection, and silence;
— every object, event, state, relationship, question, answer, unknown, uncertainty, and change — regardless of whether it originates outside N.H or inside N.H's own processing;
— every action, preparation, suggestion, and non-action;
— every failure, error, timeout, gap, interruption, and successful completion;
— every use of any record by any component — including retrieval, reading, filtering, ranking, passing to the mouth, accepting, ignoring, questioning, or connecting;
— every examination or questioning of a prior record or prior log, including recursive examination of N.H's own logs, decisions, and understandings.

**WHAT EVERY RECORD MUST CONTAIN.** A record is not complete unless it preserves:
— what component or process produced it;
— what the operation, event, or state was;
— when it occurred;
— what inputs were supplied and from where;
— what was decided, produced, or output;
— what was omitted, rejected, failed, or left unknown, and why;
— what alternatives existed and were not chosen;
— what uncertainty remained;
— what connections to other records are established or proposed;
— what it was used for, when, by which component, and with what result — every time it is used.

**ACTIVE USE, NOT PASSIVE AUDIT.** Records are not passive audit material sitting in a separate log. Every record is part of N.H's living memory and is subject to the same retrieval, connection-building, reading, interpretation, and reread rules as any other stored material. N.H must be able to internally retrieve, question, connect, and interpret its own processing history the same way it retrieves, questions, connects, and interprets its memory of Ness's life.

**RECURSIVE SELF-EXAMINATION.** N.H must be able to examine its own logs, its own Meaning Engine behavior, its own past retrieval choices, its own past acceptances and rejections, and its own prior understandings — and use the results of those examinations to inform present and future processing. Every such examination that actually occurs is itself an operation that must be recorded, connected, and available for further examination.

Recursive examination is triggered — not automatic. N.H does not continuously and automatically examine every log of every log merely because the records exist. A recursive examination occurs when triggered by: a relevance signal in the current processing context; a system purpose that genuinely requires it; a design rule that specifies it; a detected inconsistency or anomaly; a question from Ness; or another explicitly authorized trigger. When a trigger occurs, the examination runs as deep as the trigger genuinely requires. There is no fixed maximum depth when deeper examination serves a real purpose. There is also no uncontrolled automatic recursion in the absence of a trigger.

Every recursive examination that is triggered and actually performed must be recorded the same way as any other operation: what triggered it, what records were examined, what was found, what was done with the result, what remained uncertain, and what connections were established.

**USE RECORDS — LOGGING EVERY USE AND NON-USE.** Every time N.H uses a record — retrieves it, reads it, passes it to the mouth, filters it, accepts it, rejects it, ignores it, connects it to something, or examines it — that use event must itself be recorded. The use record must state: which record was used; which component used it; what the record was used for; what decision or output it contributed to; what the outcome was; what alternatives were not used and why; what remained uncertain. When a record is not used in a context where it was a candidate — when it was evaluated and set aside, or when a retrieval returned it but it was not passed forward — that non-use must also be recorded with the reason.

**APPEND-ONLY GROWTH AROUND EVERYTHING.** No record is ever overwritten, erased, or replaced. New understanding, correction, reinterpretation, or additional evidence grows around the existing record as a new linked layer. The original record stays exactly as it was. The growth is the memory. See also §0 (memory only adds) and §7Q (deletion redefined as visibility and access-path removal only, not functional removal). True destruction — permanent erasure of content from all storage locations — is prohibited inside N.H. Hiding or removing something from visible output does not automatically remove its internal usability unless Ness separately instructs that.

**CONNECTIONS ARE MANDATORY, NOT OPTIONAL.** Every record must be connected to every other record it evidentially depends on, every record it was produced from, every record it was used alongside, and every record that contradicts, confirms, or qualifies it. A record that exists without connections is incomplete. Connection-building is part of recording, not a separate later step.

**INVISIBILITY IN THE CHAT SURFACE IS NOT AN EXCEPTION.** §13 requires that N.H's internal mechanism is invisible in the chat surface and never narrated there. That rule governs what is shown to Ness in the chat — it does not exempt any internal operation from being recorded. Invisible-in-chat and recorded-internally are always both true simultaneously.

**RELATIONSHIP TO PRIVACY, SENSITIVE CONTENT, AND PROTECTED STORAGE.**

The universal recording law and the privacy and security rules are compatible. They operate on different dimensions. The universal recording law governs existence of records — every event, operation, handling decision, and use must be permanently recorded and connected. Privacy and security rules govern visibility and access to records — who and what may reach them, in what form, for what purpose, under what authorization. A record can exist permanently and still be unreachable through ordinary visible output.

**Two protection levels, not one.** Not all sensitive material requires the same level of protection. N.H uses two distinct levels.

**Level 1 — Live secrets and credentials.** Raw content at this level must never be displayed, quoted, exported, summarized, or exposed in raw form under any circumstance, including to Ness during ordinary interaction. Authorized system functions may process the raw content temporarily inside a protected execution boundary when genuinely required. Only the minimum safe result may leave that boundary, and that result must not reconstruct, reveal, or allow inference of the raw content. Every access is logged in the ordinary operational log through the opaque protected-record identifier: which function, when, why authorized, what type of operation occurred, what the minimum result was, and whether access succeeded, failed, or was refused. The raw content and the physical details of the sealed area do not appear in that log entry.

**Level 2 — Private personal information.** Raw content at this level is securely stored and protected from unauthorized access. Once N.H successfully verifies that the current user is Ness, his own Level 2 material is available to him by default through normal authorized private interaction. No separate per-access permission is required for Ness to view, discuss, summarize, or use his own medical records, memories, private conversations, personal documents, or similar personal material. External exposure, exports, tool use, shared screens, third-party access, and unauthorized secondary uses remain restricted by default and require explicit authorization. Every access and use is recorded and connected.

**The full handling record is mandatory in ordinary logs for both levels.** Ordinary operational records must always state: that sensitive material was encountered; when and where; which component handled it; why it was classified as protected and at which level; the opaque protected-record identifier of the sealed item; when an authorized component accessed or used it; why that use was permitted; how it affected the result; and whether access succeeded, failed, or was refused. For Level 1 material and for any Level 2 material specifically placed into sealed isolation, the raw content and physical access details do not appear in ordinary handling records — only the opaque protected-record identifier and the required handling fields appear. For Level 2 material that has not been placed into sealed isolation, the handling record follows the same structure but the content itself may exist in ordinary protected memory and may appear in authorized visible output to Ness after successful identity verification.

**Ness's own access to content.** For Level 1, the sealed area may be designed so that even Ness cannot directly browse or extract the raw content through ordinary means — this is a protection he has chosen for himself, and the strict never-exposed-in-raw-form rule remains in force regardless of identity verification. For Level 2, once identity verification succeeds, Ness's own material is available to him through normal authorized private interaction by default. Both levels log every access through the opaque protected-record identifier.

**Existing security and identity protections are entirely unchanged.** Neither protection level replaces, weakens, or interacts with identity verification, biometric authorization, SACL, BAI, or anti-impersonation protections. Those govern who may use N.H as Ness. These protection levels govern what certain content may be reached during authorized use.

**True destruction is prohibited. Deletion is a visibility change, not erasure and not functional removal.** No content at either protection level may ever be permanently destroyed. A request to delete content results only in that content being removed from ordinary visible output paths — the content remains preserved, internally connected, and available for N.H's internal use unless Ness separately instructs that its influence must also be removed. Every deletion request, visibility-path change, and any separate influence-removal instruction is permanently recorded and connected.
...
### §7E-TSC. TEMPORARY SESSION CACHE [CONCEPTUALLY DESIGNED, NOT BUILT]

WHAT IT IS. The Temporary Session Cache (TSC) is an organized holding mode within this unified pre-ingest store, not a separate system upstream of it. Every item from a session involving participants other than Ness enters the §7E pre-ingest store from the moment of capture, carrying the `"pending_fingerprint_authorization"` blocker alongside any other applicable blockers. The TSC structural database — a transactional local store — holds session-level organization: conversation order, speaker attribution, branch relationships, BOP event references, SIA assessment event links, N.H outputs within the session, and per-item lifecycle state. This structural database organizes the §7E pre-ingest records as a coherent session without replacing, duplicating, or bypassing them.

ONE NEW BLOCKER TYPE. `"pending_fingerprint_authorization"` is added to the §7E blocker vocabulary. All existing §7E blocker rules apply to it unchanged: held material is invisible to the Meaning Engine; one blocked item never blocks unrelated ready items; silent deletion is never permitted; material may remain in `held` indefinitely. This blocker is lifted only when a valid purpose-bound BAI one-time authorization token (`"tsc_promotion:<session_id>"`) is consumed with a confirmed recognized-Ness SACL session at the moment of consumption. The exact authorization components (BAI and SACL) are specified in the accepted TSC detailed design.

WHERE ALL ITEMS ARE HELD. Conversation contributions, BOP observation roots, N.H outputs, and session metadata from a third-party session all sit in the §7E pre-ingest store under `"pending_fingerprint_authorization"`. None of this material enters the sealed long-term root store while authorization is pending. Ness's own words within a third-party session are held in the TSC where they give meaning to another person's statements — they are not excluded from the session context. The authorization unit is one completed session's cache; unrelated sealed caches are not authorized simultaneously.

SPEAKER RESOLUTION. The existing §7E speaker-resolution rule applies without modification. An item whose speaker cannot be confirmed from source carries both `"pending_fingerprint_authorization"` and `"speaker_unresolved"`. Lifting the fingerprint-authorization blocker does not lift `"speaker_unresolved"`. Speaker-unresolved items remain held until the §7E rule is satisfied — no second fingerprint is required for them once the session is already authorized. No root is ever written with `role = "unknown"`.

SEALING AND INDEFINITE WAITING. On session close, the TSC seals immediately: a `cache_sealed` security audit event is written; no new material may be added; the session becomes immutable. A sealed TSC waits indefinitely — it does not auto-expire, auto-delete, or auto-promote. If the session closes due to a crash or unexpected termination, the cache is sealed as interrupted: all material captured before the crash is preserved exactly; nothing is reconstructed. A subsequent real-conversation session starts a new TSC linked as a continuation; the two sessions remain separate immutable units.

PROMOTION AFTER AUTHORIZATION. After authorization, items with no remaining blockers — and that pass §7Q's privacy and exclusion review — proceed through the normal §7E lifecycle: `held` → `ready` → `promoting` → `promoted`, terminating in `append_root()`. Conversation order is preserved in promotion. A parent contribution's root is confirmed before any contribution that explicitly replies to it is submitted to §7E. BOP roots promote interleaved in event-time order. No additional manual Ness action is required once the session is authorized. Items with remaining non-fingerprint blockers (e.g., `"speaker_unresolved"`) wait for those blockers to clear through normal §7E rules — they do not require a new fingerprint. `append_root()` idempotency prevents duplication in all recovery and retry scenarios.

PRIVACY PRECEDENCE. §7Q's PRIVACY AND EXCLUSION PRECEDENCE rule governs TSC material. Excluded material is replaced with non-reconstructive exclusion metadata before any promotion attempt; no excluded content is held in the active TSC or in the retained safety archive.

RETAINED SAFETY ARCHIVE. After all items in a session are promoted, excluded, or stably blocked, a frozen encrypted read-only snapshot of the TSC structural database is retained as a safety and integrity reference. This archive is not an active memory source. The Meaning Engine, LMAC, §7F, and all downstream components have no read access to it during ordinary operation. It may not be used to surface the same conversation twice or increase the evidence weight of any promoted root.

INSPECTION. Private Ness inspection of a sealed TSC — using a separate purpose-bound BAI authorization distinct from the promotion authorization — is an explicitly designed inspection mode within the meaning of §7E's pre-ingest holding area clause. Inspection is read-only. It does not lift `"pending_fingerprint_authorization"`, begin promotion, change lifecycle status, or expose material to the Meaning Engine. The inspection authorization and promotion authorization are independent; consuming one does not affect the other.

WHAT TSC IS NOT. The TSC is not a second long-term memory system, a second Meaning Engine, a semantic interpretation store, or a bypass around §7E or `append_root()`. It does not make privacy decisions — it records §7Q decisions and exclusion metadata. It does not convert a third party's statement into a fact about Ness. It does not merge speaker streams or Person-Box candidates. The root schema's seven fields are unchanged; session-context metadata flows through the §7E pre-ingest record's `source_metadata` field and the TSC structural database, not as new root schema fields.

DETAILED DESIGN. The complete accepted 31-section TSC detailed design is currently preserved in the project chat and its exported design text. It is intended to be preserved as the standalone project file `NH_ACCEPTED_TSC_DESIGN_v1.md` before or as part of the authorized integration process. That design is the authoritative specification for all implementation, integration, build, and schema decisions. The rules above summarize the integration points with §7E; they do not replace or restate the full design. `NH_ACCEPTED_SECURITY_IDENTITY_DESIGNS_AFTER_BGMM.md` preserves the earlier accepted TSC foundation and the related security/identity designs the TSC depends on.
...
**Exclusion** — raw content is intercepted at the front door before it enters any ordinary visible store. A safe handling record and opaque protected-record identifier enter the operational log. The raw content is moved to sealed protected storage or a protected pre-ingest hold at the appropriate protection level — Level 1 (§0A) for credentials and live access material, Level 2 (§0A) for private personal information where sealed pre-ingest holding is required. Nothing is silently dropped. The choice of protection level at exclusion determines what access rules apply to the content thereafter.

**Sealed isolation** — raw content exists in a protected storage area that ordinary visible roots, logs, indexes, embeddings, caches, exports, chat, interfaces, and unrestricted model context cannot reach through ordinary visible output. N.H may still internally retrieve, analyze, connect, and use sealed content as part of its memory and understanding unless a separate influence-removal instruction applies. Two sub-levels apply.

*Level 1 sealed isolation:* The raw content may be processed only inside a protected execution boundary by an authorized system function. The raw content must never be displayed, quoted, exported, summarized, or exposed in raw form under any circumstance, including to Ness.

*Level 2 sealed isolation:* The raw content is protected from unauthorized visible access but is available to Ness through normal authorized private interaction once N.H successfully verifies that the current user is Ness. No separate per-access permission is required.

In both sub-levels: ordinary records carry only the opaque protected-record identifier and the full handling and use log. The physical location, encryption details, or access path of the sealed area does not appear in ordinary records. Sealed isolation is an access-control operation, not a destruction operation. No content is erased. Sealed isolation is distinct from hiding, restriction, redaction, and deletion.

**Hiding** — content exists in ordinary storage but is not displayed in normal visible views or surfaced in ordinary visible output. A presentation rule. Reversible without data recovery. The content remains in ordinary storage and is subject to ordinary operational logging. N.H may still internally retrieve, analyze, connect, and use hidden content unless a separate influence-removal instruction applies. Hiding does not move content into sealed isolation.

**Restriction** — content exists and may be physically present in ordinary storage, but access to ordinary visible output is limited by explicit rules. More granular than hiding. More targeted than deletion. Does not move content into sealed isolation. Does not remove content. N.H may still internally retrieve, analyze, connect, and use restricted content unless a separate influence-removal instruction applies.

**Redaction** — specific content within a record is removed from ordinary visible output or obscured while the containing record remains. The removed portion is replaced with a tombstone or placeholder in the visible record. Must propagate to derived visible objects. Where sealed isolation is applicable, redaction may instead mean moving the sensitive portion into the sealed area and replacing it with a safe reference and opaque identifier in the visible record — rather than removing it from N.H's internal store. The content is never permanently destroyed. N.H may still internally use the sealed redacted content unless a separate influence-removal instruction applies.

**Deletion** — a user-facing request or system event to remove material from ordinary visible output and visible access paths. Deletion inside N.H is always and only a non-destructive state change affecting visibility and visible output. It never means permanent erasure from all storage locations, and it does not automatically remove the material's influence, connections, or internal usability. Cryptographic erasure that destroys the only copy of any content is prohibited.

**★ REJECTED DESIGN — DO NOT APPLY.** An earlier draft proposed that deletion-protected material must be made completely inaccessible to all internal functions until Ness explicitly restores it. That design is rejected. It treated visibility removal as functional removal. Those are different things and must not be conflated.

**What deletion changes.** When material enters a deletion-protected state, N.H removes it from: ordinary visible views and interfaces; visible search results and chat output; exports; shared screens; external sharing; and any other ordinary visible output path through which it would normally be surfaced to Ness or others. These are visibility and visible-output changes only. They do not prevent N.H from internally retrieving, analyzing, connecting, or using the material as part of its memory and understanding.

**What deletion does not change unless Ness separately instructs it.** Unless Ness gives an explicit separate instruction that specific material must stop influencing N.H, deletion-protected material continues to: be internally retrieved, analyzed, and used by N.H's processing layers; affect Meaning Engine readings; influence internal filtering and ranking; remain connected to related information; contribute to summaries, interpretations, current understanding, and simulation; and shape N.H's internal reasoning and future outputs. Removing something from visible output does not automatically remove its internal usability. "Hide from visible output" and "make internally unusable" are separate instructions with separate effects.

**Stopping influence is a separate explicit instruction.** If Ness wants specific deletion-protected material to stop influencing N.H's internal processing — its readings, retrieval, connections, or reasoning — that must be stated as a separate explicit instruction. That instruction is recorded separately from the deletion event, and its scope, affected components, and start time must be specified. It does not follow automatically from any deletion or hiding request.

**What happens when a deletion request is received.** N.H: moves the affected material to deletion-protected state; removes it from ordinary visible output paths; writes a permanent deletion event record; and confirms the protected state. The material remains in N.H's store, internally connected, and available for internal retrieval and use under its existing protection level rules unless a separate influence-removal instruction applies.

**The deletion event record must contain:** who or which component initiated the request; when and why; what material was affected, by opaque protected-record identifier; which visible output and visible access paths were removed; which protected state replaced ordinary visible access; whether the operation succeeded or failed; whether a separate influence-removal instruction was given and if so its scope and affected components; and what the material's current status is — hidden from ordinary visible output, preserved, not destroyed, internally usable unless separately instructed otherwise.

**History record.** A history record remains for every deletion event containing: the opaque protected-record identifier; the deletion event timestamp; who or which component initiated the request; the resulting protected state; the outcome; whether a separate influence-removal instruction exists; and the verification record. This history record is not a tombstone marking an absence — it is a positive record of what happened, because the content still exists, remains internally connected, and remains internally usable. The history record does not contain the raw sensitive content and does not expose the physical location or access path of the protected storage.

**Outcome states:**
- `protected_complete` — all ordinary visible output paths removed; content confirmed present in protected storage; full event record written.
- `protected_with_declared_limits` — ordinary visible access removed from all controllable paths; one or more paths could not be verified as removed.
- `incomplete` — one or more ordinary visible output paths remain active.
- `blocked` — the state change cannot safely proceed yet.
- `failed` — an attempted state change operation failed.

For every outcome other than `protected_complete`, Ness must be told plainly: what visible access was removed, what visible output paths remain active, what could not be verified, and what further action may be possible.

FOUR SENSITIVITY TIERS.

**Sensitivity Tier 1 — General personal.** Everyday conversation, preferences, projects, general life events. Standard N.H privacy rules apply.

**Sensitivity Tier 2 — Sensitive personal.** Health information, financial details, relationship difficulties, emotional material, and things Ness has explicitly marked as private. For Ness's authenticated private use, this material is eligible by default — it is Level 2 protected personal information under §0A and is available to Ness after successful identity verification unless Ness explicitly restricted or hid it, it was placed in sealed isolation, a deletion-protected state applies, it contains non-negotiably excluded secrets, or another explicit compartment rule applies. External exposure, shared screens, exports, tool use, notifications, and secondary uses require explicit authorization under the two-stage access-control rules below.

**Sensitivity Tier 3 — Third-party and relationship data.** Information about other people. Requires separate rules from Ness's own data because other people did not consent to being in N.H.

**Sensitivity Tier 4 — Credentials, secrets, and high-harm material.** Authentication material, passwords, keys, financial credentials. This is Level 1 protected material under §0A. Must never enter ordinary roots, readings, indexes, embeddings, caches, logs, unrestricted model context, or normal backups. When encountered at capture, intercepted immediately and moved to Level 1 sealed protected storage.

DELETION IS NON-DESTRUCTIVE VISIBILITY REMOVAL. See the Deletion operation above. True destruction — permanent erasure from all locations — is prohibited. The five former deletion outcomes (`verified_complete`, `completed_with_declared_limits`, `incomplete`, `blocked`, `failed`) are superseded by the five outcome states defined in the Deletion operation above (`protected_complete`, `protected_with_declared_limits`, `incomplete`, `blocked`, `failed`). N.H must never confirm deletion is complete merely because the ordinary visible record is no longer surfaced — the history record and the protected content both remain.

CAPTURE EXCLUSION — TWO-LAYER SYSTEM.

**Layer A — Level 1: Live secrets and credentials.** Narrow by design. Covers material that could directly grant access or cause immediate serious harm if reached through ordinary visible paths: passwords, private cryptographic keys, authentication tokens, recovery codes, one-time security codes, payment-card security codes, and equivalent live access credentials. This is Level 1 protected material under §0A.

This material must never enter ordinary roots, readings, indexes, embeddings, caches, ordinary logs, unrestricted model context, or normal backups. When encountered at a front door, it is intercepted immediately and moved to Level 1 sealed protected storage or a protected pre-ingest hold governed by separate authority, encryption, and access rules. It does not pass through ordinary capture.

A safe handling record enters the ordinary operational log containing only: that material of this category was encountered; when; at which front door; which rule applied; and the opaque protected-record identifier of the sealed item. The raw content, physical location, encryption details, and access path do not appear in this record.

Authorized system functions that genuinely require access to this material may process it temporarily inside a protected execution boundary by following the opaque reference under the authorization rules for the protected area. Only the minimum safe result may leave that boundary. The raw content must never be displayed, quoted, exported, summarized, or exposed in raw form under any circumstance. Every such access is logged through the opaque identifier: which function, when, why authorized, what type of operation, what the minimum result was, whether access succeeded, failed, or was refused.

Level 1 material is never permanently destroyed. If a deletion request is received for Level 1 material, the content remains in Level 1 sealed protected storage. Ordinary visible access is revoked. The deletion event is permanently recorded. The raw content remains preserved and accessible only through the Level 1 protected execution boundary under the existing authorization rules.
...
Remaining undesigned: exact eligibility rules, purpose categories, privacy-policy evaluation order, safe-transformation rules, interface behavior, core exclusion detection methods, mixed-content separation process, backup visibility-removal and expiry mechanics, derivative-discovery methods for sealed isolation, verification procedures for protected-complete outcome state, person-specific control interface, minor-data specific rules, volume thresholds for large imports, simulation permission specifics, exact design of the protected execution boundary for Level 1 material, exact design of the sealed protected storage area.
...
35. **TEMPORARY SESSION CACHE (TSC) — CONCEPTUALLY DESIGNED, NOT BUILT.** Transactional extension of §7E's unified pre-ingest holding area for sessions involving participants other than Ness. One new §7E blocker type (`"pending_fingerprint_authorization"`). Complete 31-section detailed design accepted and currently preserved in the project chat and its exported design text; intended to be preserved as the standalone project file `NH_ACCEPTED_TSC_DESIGN_v1.md` before or as part of the authorized integration process; earlier TSC foundation and related security/identity designs preserved in `NH_ACCEPTED_SECURITY_IDENTITY_DESIGNS_AFTER_BGMM.md`. Authorization requires a valid purpose-bound BAI one-time token (`"tsc_promotion:<session_id>"`) combined with confirmed recognized-Ness SACL session at token consumption; authorization of one session's cache does not affect unrelated sealed caches. Promotion follows conversation order; speaker-unresolved items remain held until §7E's speaker-resolution rule is satisfied without requiring a new fingerprint; `append_root()` idempotency prevents duplication. Retained safety archive held after successful promotion; not an active memory source. Does not modify the root schema. Does not bypass §7E or `append_root()`. Depends on BAI and SACL components (accepted designs in `NH_ACCEPTED_SECURITY_IDENTITY_DESIGNS_AFTER_BGMM.md`, not yet patched into Master). Build sequencing relative to other components not yet decided. §7E-TSC.
