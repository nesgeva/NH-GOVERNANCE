...
# Synced to: NH_MASTER-19_CORRECTED_v8.md (June 28 2026)
# Hash of authoritative Master: [to be computed after Master adoption]
# Supersedes: NH_DECISION_DEFAULTS-S19_v2_2.md
#
# CANDIDATE — pending adoption by Ness after Master v8 review.
# Supersedes NH_DECISION_DEFAULTS-S19_v2_2.md and all prior Decision Defaults versions.
#
...
#
# CHANGES FROM v2_2: (1) §0 BEHAVIORAL OVERRIDES — new item added:
# full-transparency and living-record law enforcement rule. (2) §6
# FIRST-CLASS PRINCIPLES — DELETION principle updated to match new §7Q.
# (3) §3 SETTLED DESIGN — §7Q entry updated to reflect six operations,
# two protection levels, and no-destruction rule.
...
- **Full-transparency and living-record law (Master §0B) applies to every design and build decision.** Every component designed or built must include explicit, mandatory recordkeeping for its own internal operations — not only for what it stores externally, but for every internal retrieval it makes, every context choice, every acceptance or rejection, every omission, every failure, every use of a prior record, and every non-use of a candidate record. A component whose internal operations produce no traceable, connected, append-only record is incomplete by design and must not be adopted. When auditing any component, verify that the following are all recorded: what the component did; what it did not do and why; what inputs it received; what it produced; what it rejected or omitted; what uncertainty remained; what prior records it used; what prior records it evaluated and did not use and why; and what the outcome was. Silent operations are prohibited. The operational record is part of N.H's living memory, subject to internal retrieval, connection-building, reading, interpretation, and triggered recursive self-examination — not a passive audit trail. Every use of an operational record must itself be recorded. N.H must be able to examine its own past behavior the same way it examines Ness's past. Hiding, restricting, or removing material from visible output does not remove its internal usability or its presence in operational records unless Ness separately instructs that.
...
- **Privacy, deletion, sensitive-data (§7Q — PARTIALLY CONCEPTUALLY DESIGNED):** six operations (exclusion, sealed isolation, hiding, restriction, redaction, deletion); four sensitivity tiers; two protection levels (Level 1: live secrets — sealed execution boundary, never exposed in raw form; Level 2: private personal information — available to Ness after identity verification by default); deletion is non-destructive visibility removal only — true destruction prohibited; five outcome states (`protected_complete`, `protected_with_declared_limits`, `incomplete`, `blocked`, `failed`); history record replaces tombstone — content remains preserved and internally usable; stopping influence requires separate explicit instruction; capture exclusion two-layer system; third-party baseline; two-stage access control; **private access for Ness is open by default** — sensitive, emotional, medical, or uncomfortable material is not blocked merely because of its subject; external exposure and secondary uses are restricted by default. (§7Q, §0A, §0B)
...
- **EVERYTHING IS RECORDED, CONNECTED, AND ACTIVELY USABLE.** Every external input and every internal N.H operation — every retrieval, reading, acceptance, rejection, omission, failure, use, non-use, and recursive self-examination — must be permanently recorded, connected, and available as living memory. Records are not passive audit material. Silent operations are prohibited. True destruction is prohibited. (§0B)
...
- **DELETION IS NON-DESTRUCTIVE VISIBILITY REMOVAL.** True destruction is prohibited. Deletion moves material out of ordinary visible output only. Content remains preserved, internally connected, and internally usable unless Ness separately gives an explicit influence-removal instruction. Stopping influence is a separate instruction with its own scope and record. Six operations. Five outcome states. History record, not tombstone. (§7Q, §0A, §0B)
...
- **THE PURE TAPE PRESERVES eligible events append-only** — subject to capture exclusions and the two protection levels. "Append-only" means nothing is erased. Level 1 material is sealed from ordinary access but preserved. Level 2 material is protected from unauthorized access but available to Ness after identity verification. (§0A, §7Q)
