...
- **Superseded draft (the base of this correction):**
...
- **Minimum-gap anchor (a wait floor, never a deadline):**
  ...
  outcome record.**
- **Episode deadline anchor (the real stop deadline):**
  `episode_deadline = episode_first_failure_record.committed_at +
  elapsed_time_max[deadline class]` [proposed] — **the elapsed-time
  clock begins when the first attempt fails and the retry episode
  begins**: the episode's first durable failure record is the clock's
  start, and its recorded `schedule_class` fixes the episode's deadline
  class once (7 minutes live chat; 15 minutes background/nightly). One
  clock per episode — a continuation episode (§10) has its own clock,
  beginning at that episode's own first durable failure. Recovery
  recomputes the identical deadline from the same records; no second
  clock, no averaging, no extension.
...
gate is reached; **the episode deadline is reached** — while waiting
(the next retry never starts; truthful **time exhaustion** is recorded)
or while an attempt is running (that attempt finishes under its
governing source seam — no forced cancellation, no override of the
seam's own timeout and terminal rules — and no further retry is admitted
afterward); the one careful retry was rejected again; or an early stop
was recorded — with no committed success.
...
- **Crash recovery (lookup-first, on top of accepted rows 1–19):**
  ...
  then proceed (row 2 pattern); a crash between change-record creation
...
| **Episode deadline reached while waiting** | The next retry never starts; **truthful time exhaustion** recorded; exhaustion disposition (§9) |
...
the minimum-gap anchors, and the episode deadline anchor are defined
with the declared storage and consumption (§6); admission requires the
...
durable rejection record as the per-case reference (§7); early stop
...
**File created:**
`NH_B9_RETRY_VALUES_WIRING_INTO_B9_B10_BHOLD_B24_v1_1_CANDIDATE.md` —
the only file created by this task item, from the verified v1_0, which
is preserved unchanged as historical candidate provenance. **Nothing
existing was
...
further retry — PASS. Original attempt counts as attempt 1; at most 2
technical retries; counts, gap anchors, and the episode deadline from
durable records only — PASS. Careful retry: durable rejection record
first; exactly one; immediate from durable readiness; realizes — never
...
explanation and evidence references; repetition alone is never a real
change — PASS. **Every record is truly append-only**: no
...
*End of `NH_B9_RETRY_VALUES_WIRING_INTO_B9_B10_BHOLD_B24_v1_1_CANDIDATE.md`
