...
**Revision note:** v0_5 = v0_4 corrected for the one verified issue in ChatGPT's fourth audit (Part K.5). v0_4 = v0_3 corrected for the four verified issues in ChatGPT's third audit (Part K.4). v0_3 = v0_2 corrected for the seven verified issues in ChatGPT's second audit (Part K.3). v0_2 = v0_1 corrected for the eleven issues of the first audit (Part K.2). v0_1 through v0_4 are preserved unchanged as history. Scope not broadened.
**Prepared:** 2026-09-16, by Claude, on Ness's direct request following the ChatGPT-prepared six-point instruction of the same date.
...
| 85 accepted files (81 at the first survey; +4 AIC/UDOK on 2026-09-16) | `04_ACCEPTED_STANDALONE_DESIGNS/` | Pass 2 done at package level (Part L); decision-by-decision rows (Pass 2b) pending |
...
## PART A — THE IDENTIFIER SCHEME (proposal, awaiting Ness's approval)
...
|---|---|---|---|---|
| NHD-WR-20260721-1 | When a new room is created, Ness or N.H may choose: blank, built from memories, or mixed. | `ness_decision`, no package | NHD-M19C-3, -4 | Master §19C line 3079–3080; Map v1.6 line 638 |
| NHD-WR-20260721-2 | Ness's direct choice overrides N.H's automatic choice. | `ness_decision`, no package | NHD-M19C-4 | same |
| NHD-WR-20260721-3 | A clear voice command changes how the room begins immediately, without another confirmation. | `ness_decision`, no package | NHD-M19C-5 | same |

Repo check (2026-09-16): no file in the **GitHub repository** contains these three decisions.

**Correction (same day, after Ness uploaded the Unreal Engine 5 package):** the three decisions ARE carried in a file — `NH_DECISION_PACKAGE_A19_UNREAL_ENGINE_5_LOCAL_WORLD_WONDER_RUNTIME_v1.md` (2026-08-14) §2.2 "Existing direct Ness room-start decisions," lines 90–103, in the same wording. That file was missing from GitHub until Ness pushed it on 2026-09-16 (commit `ca5713f`; bytes match the recorded hash). So the decisions are **not chat-only**; they are file-recorded in a non-authoritative package that is now in the repo. Status stays `ness_decision`; still no accepted A19 package; Master §19C and Map v1.6 still show them open. Ness is not asked to decide them again.
...
|---|---|---|---|
| NHD-WR-20260916-2 | As transcribed from Ness's voice on 2026-09-16: he once considered using Claude Code and ChatGPT for building N.H; he no longer wants to; he prefers to stay with Cursor because he can be more precise with it. | **`needs_ness_confirmation`** — ChatGPT's audit reports conflicting later records (coding through GitHub; Claude Code still in use). Voice transcription may also have mis-heard. Not treated as a standing decision until Ness confirms in writing. | Consistent with the standing role division if confirmed. No implementation is authorized by this note. |
...
## PART L — PASS 2: ACCEPTED PACKAGE REGISTER (`04_ACCEPTED_STANDALONE_DESIGNS/` — 81 files when surveyed on 2026-09-16; **85 after the AIC/UDOK pushes the same evening** — plus the accepted sources that physically sit in `05_ACTIVE_CANDIDATE/`)
...
| NHD-B9 | B9 retry state architecture | 1 | v1_0 (04; copy also in 05) | closure record v1_0, Jul 6 2026, PASS | uses B16-n refs | "Try again safely" — bounded, idempotent, no endless retry |
...
| NHD-B9W | B9 retry values wiring into B9/B10/B-HOLD/B24 | 1 | v1_4 (04; v1_0–v1_3 historical in 05) | closure record v1_0, Jul 12 2026, PASS | section nos. | Ness-decided retry values carried into the four consumers |
...
| NHD-A25 | A25 reread-mode assignment policy | 1 | v1_2 (04; v1_0–v1_1 historical in 05) | closure record v1_0, Jul 11 2026, PASS | section nos. | Ness-decided reread modes; assignment producer left open |
...
| NHD-A4 | A4 relevance-mode declaration policy | 2 | v1_1 (05 — accepted via Bundle 2) | acceptance record v1_0, Jul 9 2026 (no explicit PASS wording in the record) | — | Option C: one shared relevance language + mandatory per-component declarations |
| NHD-B1 | B1 context retrieval parameter architecture | 2 | v1_0 (05 — accepted via Bundle 2) | acceptance record v1_0, Jul 9 2026 (no explicit PASS wording in the record) | — | Two always-separate retrieval channels; no values chosen |
...
| NHD-BU3 | Bundle 3 story layer / person-boxes / themes / clashes (A5, A9, A14, B2, B3, B4, B19, B23, B25, B28, B-AFFIRM) | 3 | **v1_2 — physically in `05_ACTIVE_CANDIDATE/`** | acceptance record v1_0, Jul 10 2026 | section nos. | Clash never closes; six append-only theme actions; B-AFFIRM narrowed to reading accept/reject |
| NHD-BU4 | Bundle 4 living state / computed view / action / world model (A6, A8, A18, B5, B6, B8, B27, B-INT-9) | 4 | **v1_3 — physically in `05_ACTIVE_CANDIDATE/`** | acceptance record v1_0, Jul 11 2026, PASS | `A6-1…A6-11` | Eleven Living-State-Web decisions; action-risk mapping; layered world model (nine distinctions); 21 schemas; 17 record structures |
...
| NHD-LDMH-P | Live dual-model handoff bundle placement & dependency record | 7/8 | v1_1 (05) | acceptance record v1_0, Jul 13 2026, PASS | section nos. | Bundle 7 designs the model roles; Bundle 8 builds the connected handoff; no model chosen |

### L.2 Findings from Pass 2 (facts, not judgments)
- **Placement ≠ status, confirmed twice:** the accepted Bundle 3 (v1_2) and Bundle 4 (v1_3) sources live in `05_ACTIVE_CANDIDATE/`, while their acceptance records live in `04_ACCEPTED_STANDALONE_DESIGNS/`. A reader who trusts folder names would call them unaccepted. They are accepted.
...
- **Historical versions kept beside accepted ones** (A25→B10 v1_0/v1_1 in 04; B9 wiring v1_0–v1_3 and A25 v1_0–v1_1 in 05). The receipt names the accepted version; the others are provenance. Index cites only the receipt-named version as `accepted_package`; the rest as `superseded`.
- **Bundle 7 and Bundle 8 have no accepted packages.** A17 and A22 are accepted standalone policies in the A19 area; A19 itself has checkpoints only (Pass 3).
...
- **N.9:** the ten-point direction and §2.2 → `ness_decision`; all blueprint sections → `assistant_proposal` until Ness accepts the §50 scope.
...
Unreal Engine 5 is the selected World/Wonder runtime direction (not implemented) · Wonder is being designed directly, section by section; Hebrew in conversation, English in records · A17 accepted, formal closure awaits the receipt audit · **AIC and UDOK receipts** establish acceptance of their exact source bytes for limited scopes · **Memory Fabric remains inactive/unaccepted** · H1753 replaces H1752, H1763 replaces H1762.
...
| NHD-UE5-50 | §50 | Its own next step: preserve → audit → accept for the narrow runtime-direction scope if Ness chooses → consumed by A19/Bundle 7/Bundle 8 → implement only under later authorization. **No audit or acceptance record for it exists in the repo.** |
Sections §5–§8, §11–§14, §25–§39 (architecture, bridge protocol, command/event vocabularies, logging, recovery, performance profiles, Unreal project structure, components, prototypes, build sequence, test suite) are blueprint material: `assistant_proposal` until Ness accepts the scope in §50.
...
|---|---|---|---|
| The three July 21 room-start decisions | Claude working records 2026-07-21 (two chats) | `ness_decision` — **now also file-recorded** in the UE5 package §2.2 (pushed to GitHub 2026-09-16) | Recovered; no longer chat-only. Still needs its A19 package. |
...
| Cursor as the implementation tool | This conversation, 2026-09-16 (voice transcription) | **`needs_ness_confirmation`** — ChatGPT's records conflict | Part G.3. |
| Chat interface correction (NHD-WR-20260916-3) — normal scrolling conversation · ChatGPT-like basic behaviour · full-window PC chat, with smaller presentation when appropriate · centred conversation area · Ness's messages on the right in rounded boxes · N.H's replies on the left as plain text, away from the far-left screen edge · colours undecided · extra features deferred until later | Ness's 2026-09-16 instruction to Claude; Ness's earlier correction to ChatGPT | `ness_decision`; **no project file carries it yet** | Part N.1 (12.1/12.2 superseded); P.3 record task. |
...
| Decision | Pointers |
|---|---|
| Room start: blank / from memories / mixed; Ness's choice wins; voice changes it instantly (NHD-WR-20260721-1/2/3) | Claude working record 2026-07-21 · UE5 package §2.2 (repo) · answers Master §19C Q3–Q5 · Map v1.6 line 638 still lists as open |
| Chat interface | A19 checkpoint v1.1/v1.2 §12 (three-area prototype — **superseded**) · Ness's later correction (NHD-WR-20260916-3 — current; no file yet) |
| Unreal Engine 5 as runtime (NHD-UE5-1) | UE5 package §1/§49 · continuation context · A19 checkpoints §11 · `NH_DESIGN_ANSWERS.md` header |
...
| Master §19C Q1–Q5 open | Q3–Q5 answered (NHD-WR-20260721); Q1 partly touched by NHD-A19CP-7 (Create New World → blank white room); Q2 still open |
...
| Item | Class | Owner |
|---|---|---|
| Identifier scheme (Part A) | approval | Ness |
| Chat interface: record NHD-WR-20260916-3 in an A19 file and mark checkpoint §12 superseded | `missing_source` (record) | ChatGPT instruction → Claude candidate |
...
| UE5 package: audit + acceptance of its narrow runtime-direction scope (its §50) | `accepted_package` pending | ChatGPT audit → Ness |
...
### M.1 Exact duplicates — 8 files in `05_ACTIVE_CANDIDATE/` that are byte-identical to the accepted copy in `04_ACCEPTED_STANDALONE_DESIGNS/`
...
The `05/` copies add nothing; the `04/` copies are the receipt-named accepted sources.

### M.2 Superseded earlier versions of packages that were later accepted at a higher version (all in `05/` unless noted)
...
| Bundle 3 completion v1_0, v1_1 (Jul 10) | v1_2 (05 — see M.3) | acceptance record names v1_2 |
| Dual-model placement record v1_0 (Jul 13) | v1_1 (05 — see M.3) | acceptance record names v1_1 |
...
### M.3 Accepted sources that physically sit in `05_ACTIVE_CANDIDATE/` (folder name misleads; these are NOT old)
Bundle 3 completion v1_2 · Bundle 4 completion v1_3 · A4 v1_1 · B1 v1_0 · A1 design-closure v1_1 · Dual-model handoff placement record v1_1. Each has its acceptance/closure record in `04/`. Reading the folder name alone would wrongly mark all six as unaccepted.
...
- **Local layout is already ahead of GitHub on placement:** locally, the six accepted sources listed in M.3 sit in `04_ACCEPTED…`; on GitHub they still sit in `05_ACTIVE_CANDIDATE/`.
...
- 8 files are exact copies you don't need twice.
- About 17 files are earlier drafts of things you later accepted at a higher version (a few are deliberately kept as history by their own receipts).
- 6 accepted files are in the "candidate" folder and look unaccepted but aren't.
- Everything dated Sep 15 is your current working material.
- Nothing here is a design conflict. It is housekeeping, and it is yours to decide.
...
1. Approve the identifier scheme (Part A).
...
2b. Confirm or withdraw G.3 (Cursor vs Claude Code/ChatGPT for building) — in writing, because ChatGPT's records conflict with the voice transcription.
