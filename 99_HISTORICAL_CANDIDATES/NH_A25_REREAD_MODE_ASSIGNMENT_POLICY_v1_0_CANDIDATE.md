# NH_A25_REREAD_MODE_ASSIGNMENT_POLICY_v1_0_CANDIDATE.md
...
mode describes **the relationship between a piece of new information and
an existing old reading**; it is assigned by that relationship and by
nothing else.
...
- **`wider-context` reread mode** — assigned when the new information
  **may materially change the surrounding meaning or context of the old
  ...
  content is not directly contradicted or corrected, but what it means
  may shift because the context around it has materially changed.

**The boundary between the modes** is exactly the decided distinction:
*direct bearing on the old reading's own content* versus *material change
to the meaning of its surroundings without direct contradiction.* No
finer sub-boundary, precedence rule, tie-break, or overlap-resolution
mechanism is invented here; where a real case genuinely presents both
relationships, nothing in this package forces one label, and the settled
honest-recording rules govern what is written.
...
- **No new trigger types.** The settled §7H trigger set — manual;
  condition-based (materially relevant new information; time alone never
  qualifies); scheduled automatic retry (temporary technical conditions
  only, never reinterpretation) — is unchanged. A25 adds no fourth
  trigger and removes none.
...
- **Master §7H** — the three triggers, the record set, and the
  new-reading-beside-old rule stand exactly as settled. §7H's scope
...
boundary are defined strictly from that decision (§5); the not-blanket,
recorded-reason, no-new-trigger, append-only, and nothing-quantitative
boundaries hold (§6); the settled §7H and accepted B10 architecture stand
...
**File created:** `NH_A25_REREAD_MODE_ASSIGNMENT_POLICY_v1_0_CANDIDATE.md`
— the only file created by this task. First version; no prior A25
standalone package exists. **Nothing existing was modified:** Master V10,
...
mode-slot default explicitly left standing — PASS. Append-only and
...
*End of `NH_A25_REREAD_MODE_ASSIGNMENT_POLICY_v1_0_CANDIDATE.md` —
