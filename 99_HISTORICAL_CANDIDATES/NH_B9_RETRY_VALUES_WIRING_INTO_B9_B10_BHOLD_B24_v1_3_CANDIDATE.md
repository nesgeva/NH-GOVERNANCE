...
- **Correction base:**
  ...
  SHA-256
  `7c2a3cbbb2eefc043ea2ce0cb46702a492682d108146479fcd7bfbf93b683fc2`;
  888 lines; 53,636 bytes — **recalculated; matched the correction
...
budget-config version, gap-anchor evidence, and deadline evidence used.
Concurrent admissions still admit exactly one winner (accepted R1);
losers observe and append nothing. **No reset, collision, or
skipped-bound interpretation is possible:** the permanent next number,
the episode ordinal, the per-episode count, and the correct
ordinal-indexed gap are all reconstructed lookup-first from durable
records — never from an in-memory-only counter (§5, §14).
...
- **A new bounded episode, never an extension:** the new episode carries
  ...
  positions restart at 1 (§5): its first admitted re-attempt is
  ...
  (§6)** — the exhausted episode's records stand untouched and are
...
- **Idempotency:** each admission atomically binds {`retry_group_id`,
  ...
  gap-anchor evidence, deadline evidence}; a repeat identical admission
...
ordinal, and the exact budget, gap, and deadline evidence (§5, §7);
...
**File created:**
`NH_B9_RETRY_VALUES_WIRING_INTO_B9_B10_BHOLD_B24_v1_3_CANDIDATE.md` —
the only file created by this task item, from the verified v1_2, which
is preserved unchanged as historical candidate provenance (v1_0 and
v1_1 likewise preserved). **Nothing
existing was
...
exact budget, gap, and deadline evidence with one winner; everything
reconstructed from durable records — no reset, collision, skipped-bound
interpretation, or in-memory-only counter** — PASS. At most 2 technical
...
*End of `NH_B9_RETRY_VALUES_WIRING_INTO_B9_B10_BHOLD_B24_v1_3_CANDIDATE.md`
